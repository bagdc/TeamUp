...
if ( ! class_exists( 'Bp_Add_Group_Types_Admin' ) ) :
	...
	class Bp_Add_Group_Types_Admin {
		...
		public function bpgt_admin_settings_page() {
			$tab = ( filter_input( INPUT_GET, 'tab' ) !== null ) ? filter_input( INPUT_GET, 'tab' ) : 'bpgt-general';
			?>
		<div class="wrap">
			<div class="bpgt-header">
				<?php echo do_shortcode( '[wbcom_admin_setting_header]' ); ?>
				<h1 class="wbcom-plugin-heading">
					<?php esc_html_e( 'Group Types - BuddyPress', 'bp-add-group-types' ); ?>
				</h1>
			</div>
			<?php settings_errors(); ?>
		...
		public function bpgt_plugin_settings_tabs() {
			...
			if ( bp_is_network_activated() ) {
				$group_url = get_admin_url( bp_get_root_blog_id(), 'edit.php?post_type=bp_group_type' );
			} else {
				$group_url = bp_get_admin_url( add_query_arg( array( 'post_type' => 'bp_group_type' ), 'edit.php' ) );
			}

			$current_tab = ( filter_input( INPUT_GET, 'tab' ) !== null ) ? filter_input( INPUT_GET, 'tab' ) : 'bpgt-general';
			...
			echo '<a class="nav-tab" id="bpgt-create-group-type-link" href="' . $group_url .'">' . esc_html__( 'Create Group Types', 'bp-add-group-types' ) . '</a>';
			echo '</h2></div>';
		}
		...
		public function relocate_cpt_admin_screen() {
			if ( is_network_admin() && bp_is_network_activated() ) {
				$group_url = get_admin_url( bp_get_root_blog_id(), 'edit.php?post_type=' . $this->post_type );
			} else {
				$group_url = bp_get_admin_url( add_query_arg( array( 'post_type' => $this->post_type ), 'edit.php' ) );
			}
			...
			<tbody>
				...
				<tr>
					...
					<td>
						...
						?>
						<textarea class="wp-editor-area" autocomplete="off" name="content" id="content" aria-hidden="true" style="width:100%; height: 100px;"><?php echo $post->post_content; ?></textarea>
					</td>
				</tr>
			</tbody>
		</table>
		...
		<p><?php _e( 'To display all groups with this group type on a dedicated page, add this shortcode to any WordPress page.', 'bp-add-group-types' ); ?></p>
		<code id="group-type-shortcode"><?php echo '[bp_group type="' . $key . '"]'; ?></code>
			<?php
		}
