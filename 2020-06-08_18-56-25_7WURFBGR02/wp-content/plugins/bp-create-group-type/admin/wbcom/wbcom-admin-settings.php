...
if ( !class_exists( 'Wbcom_Admin_Settings' ) ) {
	...
	class Wbcom_Admin_Settings {
		...
		public function wbcom_all_free_plugins() {
			$free_plugins = array(
				'0'	 => array(
					'name'			 => esc_html__( 'Custom Font Uploader', 'bp-add-group-types' ),
					'slug'			 => 'custom-font-uploader',
					'description'	 => esc_html__( 'It also allows you to upload your own custom font to your site and use them using custom css.', 'bp-add-group-types' ),
					'status'		 => $this->wbcom_plugin_status( 'custom-font-uploader' ),
					'wp_url'		 => 'https://wordpress.org/plugins/custom-font-uploader/',
					'icon'			 => 'fa fa-upload'
				),
				'1'	 => array(
					'name'			 => esc_html__( 'BuddyPress Create Group Type', 'bp-add-group-types' ),
					'slug'			 => 'bp-create-group-type',
					'description'	 => esc_html__( 'It will help to create group type for BuddyPress Groups.', 'bp-add-group-types' ),
					'status'		 => $this->wbcom_plugin_status( 'bp-create-group-type' ),
					'wp_url'		 => 'https://wordpress.org/plugins/bp-create-group-type/',
					'icon'			 => 'fa fa-sitemap'
				),
				'2'	 => array(
					'name'			 => esc_html__( 'BuddyPress Member Reviews', 'bp-add-group-types' ),
					'slug'			 => 'bp-user-profile-reviews',
					'description'	 => esc_html__( 'This plugin allows only site members to add reviews to the buddypress members on the site and even rate the member’s profile out of 5 points with multiple review criteria.', 'bp-add-group-types' ),
					'status'		 => $this->wbcom_plugin_status( 'bp-user-profile-reviews' ),
					'wp_url'		 => 'https://wordpress.org/plugins/bp-user-profile-reviews/',
					'icon'			 => 'fa fa-user'
				),
				'3'	 => array(
					'name'			 => esc_html__( 'BuddyPress Group Reviews', 'bp-add-group-types' ),
					'slug'			 => 'review-buddypress-groups',
					'description'	 => esc_html__( 'This plugin allows the BuddyPress Members to give reviews to the BuddyPress groups on the site. The review form allows the users to give text review, even rate the group on the basis of multiple criterias.', 'bp-add-group-types' ),
		...
		public function wbcom_all_paid_plugins() {
			$paid_plugins = array(
				...
				'1'	 => array(
					'name'			 => esc_html__( 'BuddyPress Polls', 'bp-add-group-types' ),
					'description'	 => esc_html__( 'Use BuddyPress Polls plugin to create polls inside the activity, let your user response to your polls. Members can create pools like activities, easily votes on them.', 'bp-add-group-types' ),
					'download_url'	 => 'https://wbcomdesigns.com/downloads/buddypress-polls/?utm_source=wporg&utm_medium=plugins&utm_campaign=bp',
					'icon'			 => 'fa fa-bar-chart'
				),
				'2'	 => array(
					'name'			 => esc_html__( 'BuddyPress Hashtags', 'bp-add-group-types' ),
					'description'	 => esc_html__( 'Allows members to use hashtags in BuddyPress activities and bbPress topics and ready with widgets.', 'bp-add-group-types' ),
					'download_url'	 => 'https://wbcomdesigns.com/downloads/buddypress-hashtags/?utm_source=wporg&utm_medium=plugins&utm_campaign=bp',
					'icon'			 => 'fa fa-hashtag'
				),
				'3'	 => array(
					'name'			 => esc_html__( 'BuddyPress Profanity', 'bp-add-group-types' ),
					'description'	 => esc_html__( 'Use BuddyPress Profanity plugin to censor content in your community! Easily Censor all the unwanted words in activities, private messages contents by specifying a list of keywords to be filtered.', 'bp-add-group-types' ),
					'download_url'	 => 'https://wbcomdesigns.com/downloads/buddypress-profanity/?utm_source=wporg&utm_medium=plugins&utm_campaign=bp',
					'icon'			 => 'fa fa-ban'
				),
				'4'	 => array(
					'name'			 => esc_html__( 'BuddyPress Private Community Pro', 'bp-add-group-types' ),
					'description'	 => esc_html__( 'This plugin offers a lockdown for BuddyPress Component and will ask users to log in go further to check profile or any other protected details.', 'bp-add-group-types' ),
				...
				'5'	 => array(
					'name'			 => esc_html__( 'BuddyPress Profile Pro', 'bp-add-group-types' ),
					'description'	 => esc_html__( 'This plugin gives you the power to extend BuddyPress Profiles with repeater fields and groups. You can easily add multiple field groups and display them at member’s profile.', 'bp-add-group-types' ),
				...
				'6'	 => array(
					'name'			 => esc_html__( 'BuddyPress Sticky Post', 'bp-add-group-types' ),
					'description'	 => esc_html__( 'This will make it easier for the site administrator to make the pinned activity item recognized first by the community whenever they visit the site.', 'bp-add-group-types' ),
				...
				'7'	 => array(
					'name'			 => esc_html__( 'BuddyPress Auto Friends', 'bp-add-group-types' ),
					'description'	 => esc_html__( 'Allow the site admin to select global friends for all his members with BuddyPress Auto friends plugin.', 'bp-add-group-types' ),
				...
				'8'	 => array(
					'name'			 => esc_html__( 'BuddyPress Quotes', 'bp-add-group-types' ),
					'description'	 => esc_html__( 'BuddyPress quotes plugin comes with the feature to let users post their activity updates with interactive backgrounds selection such as colors and images set by the site administrator, so they can tell a more expressive story.', 'bp-add-group-types' ),
		...
		public function wbcom_enqueue_admin_scripts() {
			...
			if ( !wp_script_is( 'wbcom_admin_setting_js', 'enqueued' ) ) {
				...
				wp_register_script(
				$handle		 = 'wbcom_admin_setting_js', $src		 = BP_GROUP_TYPE_PLUGIN_URL . 'admin/wbcom/assets/js/wbcom-admin-setting.js', $deps		 = array( 'jquery' ), $ver		 = time(), $in_footer	 = true
				);
				wp_localize_script(
			...
			if ( function_exists( 'get_current_screen' ) ) {
				...
				if ( 'toplevel_page_wbcomplugins' === $screen->base ) {
					wp_enqueue_style( 'bp-add-group-types-css', BP_GROUP_TYPE_PLUGIN_URL . 'admin/css/bp-add-group-types-admin.css', array() );
					wp_enqueue_script( 'bp-add-group-types-js', BP_GROUP_TYPE_PLUGIN_URL . 'admin/js/bp-add-group-types-admin.js', array('jquery') );
        			wp_localize_script( 'bp-add-group-types-js', 'my_ajax_object', array('ajax_url' => admin_url('admin-ajax.php'), 'nonce' => wp_create_nonce( 'bp_share_nonce' )));
				}
			}
		}
		...
		public function wbcom_admin_additional_pages() {
			add_submenu_page(
			'wbcomplugins', esc_html__( 'Components', 'bp-add-group-types' ), esc_html__( 'Components', 'bp-add-group-types' ), 'manage_options', 'wbcom-plugins-page', array( $this, 'wbcom_plugins_submenu_page_callback' )
			);
			add_submenu_page(
			'wbcomplugins', esc_html__( 'Themes', 'bp-add-group-types' ), esc_html__( 'Themes', 'bp-add-group-types' ), 'manage_options', 'wbcom-themes-page', array( $this, 'wbcom_themes_submenu_page_callback' )
			);
			add_submenu_page(
			'wbcomplugins', esc_html__( 'Support', 'bp-add-group-types' ), esc_html__( 'Support', 'bp-add-group-types' ), 'manage_options', 'wbcom-support-page', array( $this, 'wbcom_support_submenu_page_callback' )
			);
		}
		...
		public function wbcom_admin_setting_header_html() {
			...
			<div id="wb_admin_header" class="wp-clearfix">
				...
				<div id="wb_admin_logo">
					<img src="<?php echo BP_GROUP_TYPE_PLUGIN_URL . 'admin/wbcom/assets/imgs/logowbcom.png'; ?>">
					...
					<ul>
						...
						<li class="wb_admin_nav_item <?php echo esc_attr( $plugin_active ); ?>">
							<a href="<?php echo get_admin_url() . 'admin.php?page=wbcom-plugins-page'; ?>" id="wb_admin_nav_trigger_extensions">
								<i class="fa fa-th"></i>
								<h4><?php esc_html_e( 'Components', 'bp-add-group-types' ); ?></h4>
							</a>
						</li>
						<li class="wb_admin_nav_item <?php echo esc_attr( $theme_active ); ?>">
							<a href="<?php echo get_admin_url() . 'admin.php?page=wbcom-themes-page'; ?>" id="wb_admin_nav_trigger_themes">
								...
								<h4><?php esc_html_e( 'Themes', 'bp-add-group-types' ); ?></h4>
							</a>
						</li>
						<li class="wb_admin_nav_item <?php echo esc_attr( $support_active ); ?>">
							<a href="<?php echo get_admin_url() . 'admin.php?page=wbcom-support-page'; ?>" id="wb_admin_nav_trigger_support">
								...
								<h4><?php esc_html_e( 'Support', 'bp-add-group-types' ); ?></h4>
							</a>
						</li>
